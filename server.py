...
isDirExist = os.path.isdir('shared_files')
if(not isDirExist): 
...
def permissionDenied(client_name):
    global clients
    otherClientName = clients[client_name]['connected_with']
    otherClientSocket = clients[otherClientName]['client']

    msg = f'Client declined your request'
    otherClientSocket.send(msg.encode())

def sendFile(client_name,file_name,file_size):
    global clients
    ...
    msg = f'\n!{client_name} wants to send {file_name} file with size {file_size} bytes, do you want to download it? y/n '
    otherClientSocket.send(msg.encode())
    time.sleep(1)
    msgDown = f'Download: {file_name}'
    otherClientSocket.send(msgDown.encode())
...
def sendTextMessage(client_name, message):
    ...
    otherClientSocket.send(final_msg.encode())
